...
> **Important DynamoDB note:** Python `float` values cannot be written to DynamoDB via boto3; numeric values must be stored as `Decimal`. The generator Lambda converts floats to `Decimal` before writing.
